...
def extract_text_from_excel_bytes(file_bytes):
    excel_file = BytesIO(file_bytes.read())
...
def upload_chunks(chunks, filename, creation_date, doc, start_id, doc_type="xlsx"):
    ...
    for idx, (chunk, vec) in enumerate(zip(chunks, vectors)):
        ...
        # Default page number or row index
        page_number = 1

        if doc_type == "pdf":
            for page_idx in range(len(doc.pages)):
                page_text = doc.pages[page_idx].extract_text()
                if page_text and first_line in clean_text(page_text):
                    page_number = page_idx + 1
                    break

        elif doc_type == "xlsx":
            # You might want to extract sheet name or row info here if needed
            page_number = 1  # or a row index if you track it
...
async def upload_pdf(file: UploadFile = File(...)):
    try:
        ...
        if file_extension == 'pdf':
            ...
            text, doc = extract_text_from_excel_bytes(file.file)
            creation_date = datetime.now().strftime("%Y-%m-%d")
        else:
            raise ValueError("Unsupported file type. Only PDF and Excel files are accepted.")

        chunks = split_text(text)
        num_uploaded = upload_chunks(chunks, filename, creation_date, doc, start_id)
